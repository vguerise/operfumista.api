...
// CORS SUPER ROBUSTO - garante headers em QUALQUER situação
function setCorsHeaders(req, res) {
  ...
  if (origin && ALLOWED_ORIGINS.has(origin)) {
    ...
    // Se não tem origin (algumas ferramentas), permite tudo
    res.setHeader("Access-Control-Allow-Origin", "*");
  } else {
    // Por segurança, mesmo se não está na lista, responde com algo
    res.setHeader("Access-Control-Allow-Origin", origin);
  }
...
// Suas instruções originais (mantidas 100%)
...
export default async function handler(req, res) {
  // ⚠️ CRÍTICO: CORS PRIMEIRO - ANTES DE QUALQUER OUTRA COISA
  setCorsHeaders(req, res);

  // Preflight OPTIONS - DEVE responder IMEDIATAMENTE
  if (req.method === "OPTIONS") {
    console.log("✅ Preflight OPTIONS - respondendo com 200");
    return res.status(200).end();
  }

  // Apenas POST permitido (além de OPTIONS acima)
  if (req.method !== "POST") {
  ...
  try {
    console.log("📥 POST recebido de:", req.headers.origin);
    ...
    try {
      ...
      // Validação
      const errors = [];
      if (!data.analise_colecao) errors.push('Falta analise_colecao');
      if (!data.recomendacoes) errors.push('Falta recomendacoes');
